...
const TOOL_OPTIONS = ['', 'Einfachkernrohr', 'Schappe', 'Schnecke', 'Spirale', 'Kiespumpe', 'Ventilbohrer', 'Meißel', 'Sonde', 'Schlagrohr', 'Rollenmeißel', 'Doppelkernrohr', 'Diamantmeißel', 'Rammrotationskernrohr', 'Seilkernrohr mit Hohlbohrkrone', 'Ventilbüchse', 'Stauchrohr', 'Rammkernrohr', 'Schlagschappe', 'Hohlbohrschnecke', 'Hohlmeißel', 'Flügel,-Kreuzmeißel', 'Dreifachkernrohr', 'offene Bohrkrone', 'Bagger', 'Seilkernrohr', 'Hohlkrone', 'Vollkrone', 'Diamantkrone', 'Greifer'];
const DRILLING_METHOD_OPTIONS = ['', 'Hammerbohren', 'Kernbohren', 'Rotationskernbohren', 'Saugbohren', 'Seilschlagbohren', 'Schneckenbohren', 'Trockendrehbohren', 'Greiferbohren', 'Rammbohren', 'Rammkernbohren', 'Rammrotationskernbohren', 'Rotationsspülbohren', 'Rotationstrockenkernbohren', 'Schlagbohren', 'Vibrationsbohren'];

function emptyMeta() {
return { bezeichnung: '', date: new Date().toISOString().slice(0,10), user: '', project: '', borehole: '', location: '', device: '', note: '' };
}
...
function saveCurrentToHistory() {
  const title = `${state.meta.project || '—'} · ${state.meta.bezeichnung || '—'}`;
  const entry = {
    id: uid(),
    savedAt: Date.now(),
    title,
...
function syncMetaToUi() {
  $('meta-bezeichnung').value = state.meta.bezeichnung || '';
...
function collectMetaFromUi() {
  state.meta.bezeichnung = $('meta-bezeichnung').value || '';
...
function syncMetaAccordionMeta() {
  ...
  if (state.meta.bezeichnung) parts.push(state.meta.bezeichnung);
  if (state.meta.date) parts.push(state.meta.date);
  el.textContent = parts.join(' · ') || 'Projekt, Bezeichnung, Datum';
}
  ...
  return `
    <div class="form-grid">
      ...
      ${selectHtml({
        layerId: layer.id,
        field: 'drillMethod',
        options: DRILLING_METHOD_OPTIONS,
        value: layer.drillMethod || '',
        label: 'Bohrverfahren'
      })}
      ...
      ${quick ? '' : `
        ...
        <label class="field">
          ...
          <input class="field__input" type="range" min="0" max="100" step="5" data-field="recovery" data-id="${h(layer.id)}" value="${h(layer.recovery || 0)}" style="--val:${h(layer.recovery || 0)}%" oninput="document.getElementById('recVal-${h(layer.id)}').textContent=this.value;this.style.setProperty('--val',this.value+'%')" />
        </label>
      `}
    </div>
  `;
...
const name = b.meta.bezeichnung || b.meta.borehole || 'Neuer Bodenaufschluss';
...
function hookMetaEvents() {
  ['meta-bezeichnung', 'meta-date', 'meta-user', 'meta-project', 'meta-location', 'meta-device', 'meta-note'].forEach(id => {
